...
CPUFamily gCPUFamily = CPUFamily_Unsupported;
...
void offsets_init(void) {
    if (!(SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"17.0") && SYSTEM_VERSION_LESS_THAN(@"26.6"))) {
        printf("[-] Only supported offset for iOS/iPadOS 17.0 - 26.5.x\n");
        exit(EXIT_FAILURE);
    }
    ...
    // Set CPU family for runtime offset selection
    if (cpuFamily == CPUFAMILY_ARM_COLL) {
        gCPUFamily = CPUFamily_A17;
    } else if (cpuFamily == CPUFAMILY_ARM_TAHITI && isA17Above && !gIsA18Above) {
        gCPUFamily = CPUFamily_A17Pro;
    } else if (cpuFamily == CPUFAMILY_ARM_TUPAI) {
        gCPUFamily = CPUFamily_A18;
    } else if (cpuFamily == CPUFAMILY_ARM_TAHITI) {
        // TAHITI is used for both A17 Pro and A18 Pro; distinguish by version
        if (gIsA18Above) {
            gCPUFamily = CPUFamily_A18Pro;
        } else {
            gCPUFamily = CPUFamily_A17Pro;
        }
    } else if (cpuFamily == CPUFAMILY_ARM_DONAN) {
        gCPUFamily = CPUFamily_A18X;
    } else if (cpuFamily == CPUFAMILY_ARM_TILOS) {
        gCPUFamily = CPUFamily_A19;
    } else if (cpuFamily == CPUFAMILY_ARM_THERA) {
        gCPUFamily = CPUFamily_A19Pro;
    }
    
    printf("[CPU] gIsA18Above=%d, gCPUFamily=%ld\n", gIsA18Above, (long)gCPUFamily);
    ...
    if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"26.0")) {
        ...
        if(gIsA18Above) {
            // BUG FIX for iOS 26.0+ on A17 (iPhone 15, iPhone15,4):
        // 26.0+ was unconditionally using 0x3f0 (A18 value),
        // but A17 needs 0x3c0. Make this chip-dependent.
        if (gIsA18Above) {
            off_thread_t_tro = 0x3f0;
        } else if (gCPUFamily == CPUFamily_A17) {
            off_thread_t_tro = 0x3c0;
        } else {
            // Older chips: keep 26.0+ value
            off_thread_t_tro = 0x3f0;
        }
            off_thread_ctid = 0x4b0;
            off_thread_mutex_lck_mtx_data = 0x418+8;
